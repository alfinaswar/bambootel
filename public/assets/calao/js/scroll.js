...
function scrollSmooth(direction) {
  ...
  // Batasi scroll supaya tidak melebihi batas
...
// Inisialisasi tombol
scrollLeftBtn.addEventListener("click", () => scrollSmooth(-1));
scrollRightBtn.addEventListener("click", () => scrollSmooth(1));
updateButtons(); // pastikan tombol awal sesuai posisi
...
document.querySelectorAll("#villaWrapper > div").forEach((card) => {
  ...
  card.querySelector(".next-btn").addEventListener("click", () => {
    ...
    changeImage(imgContainer, villaImages[nextIndex], currentImg);
...
function changeImage(container, newSrc, currentImg) {
